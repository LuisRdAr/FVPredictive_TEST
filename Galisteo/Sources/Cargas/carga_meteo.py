...
if __name__ == "__main__":
    ...
    # Calculo de horas de salida y puesta del sol para los nuevos días cargados y volcado en la base
        # de datos
    ...
    fechas = pd.DataFrame(fechas_nuevas, 
                          columns = ["date"]).merge(pd.DataFrame(fechas_viejas, 
                                                                 columns = ["date"]), 
                                                    how = "left", 
                                                    indicator = True)
    ...
    # Cálculo de las horas de salida y puesta del sol para las coords dadas usando la API 
        # sunrise_sunset y volcado en la base de datos
    hora_salida_sol = []
    hora_puesta_sol = []
    ...
    for fecha in tqdm(fechas.itertuples(), total = fechas.shape[0]):
        r = requests.get('https://api.sunrise-sunset.org/json', 
                         params={'lat': lat, 
                                 'lng': lon, 
                                 'date': fecha[1].strftime("%Y-%m-%d")}).json()['results']
        hora_salida_sol.append(pd.to_datetime(fecha[1].strftime("%Y-%m-%d") + " " + r['sunrise'], 
                                              utc = True))
        hora_puesta_sol.append(pd.to_datetime(fecha[1].strftime("%Y-%m-%d") + " " + r['sunset'], 
                                              utc = True))
    daylight = pd.DataFrame({"date": fechas["date"], 
                             "sunrise": hora_salida_sol, 
                             "sunset": hora_puesta_sol})
    daylight.to_sql('daylight', 
                    engine, 
                    if_exists='append', 
                    index = False, 
                    schema = schema_name, 
