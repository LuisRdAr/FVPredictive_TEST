...
# BOOLEANA QUE INDIQUE EL FALLO DE LOS SENSORES.
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(0, count, CHUNK_SIZE), total = math.ceil(count/CHUNK_SIZE)):
        ...
        metrics["upper_lim"] = metrics["percentile_75"] + 3 * (metrics["percentile_75"] - metrics["percentile_25"])
        metrics["lower_lim"] = metrics["percentile_25"] - 1.5 * (metrics["percentile_75"] - metrics["percentile_25"])

        meteo_df["status_srl"] = np.where((meteo_df["rad_poa"] < metrics[metrics["column"] == "rad_poa"]["lower_lim"].values[0]) | 
                                            (meteo_df["rad_poa"] > metrics[metrics["column"] == "rad_poa"]["upper_lim"].values[0]), 
                                        2, 
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["rad_hor"] < metrics[metrics["column"] == "rad_hor"]["lower_lim"].values[0]) | 
                                            (meteo_df["rad_hor"] > metrics[metrics["column"] == "rad_hor"]["upper_lim"].values[0]),
                                        2,
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["rad_celda1"] < metrics[metrics["column"] == "rad_celda1"]["lower_lim"].values[0]) | 
                                            (meteo_df["rad_celda1"] > metrics[metrics["column"] == "rad_celda1"]["upper_lim"].values[0]),
                                        2,
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["rad_celda2"] < metrics[metrics["column"] == "rad_celda2"]["lower_lim"].values[0]) | 
                                            (meteo_df["rad_celda2"] > metrics[metrics["column"] == "rad_celda2"]["upper_lim"].values[0]),
                                        2,
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["temp_amb"] < metrics[metrics["column"] == "temp_amb"]["lower_lim"].values[0]) | 
                                            (meteo_df["temp_amb"] > metrics[metrics["column"] == "temp_amb"]["upper_lim"].values[0]),
                                        2,
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["temp_panel1"] < metrics[metrics["column"] == "temp_panel1"]["lower_lim"].values[0]) | 
                                            (meteo_df["temp_panel1"] > metrics[metrics["column"] == "temp_panel1"]["upper_lim"].values[0]),
                                        2,
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["temp_panel2"] < metrics[metrics["column"] == "temp_panel2"]["lower_lim"].values[0]) | 
                                            (meteo_df["temp_panel2"] > metrics[metrics["column"] == "temp_panel2"]["upper_lim"].values[0]),
                                        2,
                                        meteo_df["status_srl"])
        meteo_df["status_srl"] = np.where((meteo_df["rad_poa"] == 0) & (meteo_df["rad_hor"] == 0) & 
        ...
        hum_df = meteo_df[meteo_df["dispositivo_id"] == 42][["datetime_utc","hum_rel", "vel_viento", "dir_viento"]]
        hum_df = hum_df.dropna(subset=["hum_rel"])
        hum_df["status_srl"] = 0
        hum_df["status_srl"] = np.where((hum_df["hum_rel"] >= 0) & (hum_df["hum_rel"] <= 100) &
                                            (hum_df["vel_viento"] >= 0) & (hum_df["vel_viento"] <= 100) &
                                            (hum_df["dir_viento"] >= 0) & (hum_df["dir_viento"] <= 360), 
                                        0, 
                                        3)

        # Se unen los dataframes de irradiancia y humedad y viento
        prep_meteo_df = pd.merge(meteo_df.drop(columns=["vel_viento", "dir_viento", "hum_rel"]), hum_df, on = ["datetime_utc"], how = "left")
        ...
        prep_meteo_df["status_srl"] = np.where((prep_meteo_df["status_srl_x"] == 0) &
                                                    ((prep_meteo_df["status_srl_y"] == 3) | (prep_meteo_df["status_srl_y"] == 4)), 
    ...
    cur.close()
    conn.close()
    print("Número de registros con etiquetado por anomalía en el rango: ", prep_meteo_df[(prep_meteo_df["status_srl"] != 0) | (prep_meteo_df["status_srl"] != 1)].shape[0])
