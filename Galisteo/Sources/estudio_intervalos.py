...
def optimize_hyperparameters(space, dtrain, STALL_LIMIT = 5, MAX_EVALS_PER_RUN = 250, gamma_algo = 0.75, cv_folds = 5):
    best_loss = np.inf
    trials = Trials()
    stall_counter = 0
    num_evals = 0
...
intervalo_min = [60,30,20,15,10,5]
seeds = np.random.randint(0, 50000, size=5000)
num_mod_string = 30
sup_mod = 2
rmse_intervalos = []
mae_intervalos = []
for intervalo in intervalo_min:
    rmse_seeds = []
    mae_seeds = []
    print(f"Intervalo de {intervalo} minutos")
    ...
    print()
    print(f"RMSE para el intervalo de {intervalo} min: {np.mean(rmse_seeds)}")
    print(f"MAE para el intervalo de {intervalo} min: {np.mean(mae_seeds)}", end="\n\n")
    rmse_intervalos.append(np.mean(rmse_seeds))
    mae_intervalos.append(np.mean(mae_seeds))
sns.barplot(x = intervalo_min, y = rmse_intervalos)
plt.title("RMSE para distintos intervalos de tiempo")
plt.savefig("rmse_intervalos.png")
plt.show()
sns.barplot(x = intervalo_min, y = mae_intervalos)
plt.title("MAE para distintos intervalos de tiempo")
plt.savefig("mae_intervalos.png")
plt.show()
print(f"Intervalo con menor RMSE: {intervalo_min[np.argmin(rmse_intervalos)]}")
print(f"Intervalo con menor MAE: {intervalo_min[np.argmin(mae_intervalos)]}")
print(f"Intervalo con mayor RMSE: {intervalo_min[np.argmax(rmse_intervalos)]}")
print(f"Intervalo con mayor MAE: {intervalo_min[np.argmax(mae_intervalos)]}")
