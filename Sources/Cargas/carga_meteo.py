...
if __name__ == "__main__":
    ...
    for row in tqdm(df.itertuples(), total = df.shape[0]):
        try:
            cur.execute(f"""INSERT INTO {schema_name}.meteo_raw(
                    ...
                    rad_poa, rad_hor, rad_celda1, rad_celda2, temp_amb, temp_panel1,  
                    temp_panel2, vel_viento, dir_viento, hum_rel)
                VALUES 
                ...
                row.rad_poa, row.rad_hor, row.rad_cel1, row.rad_cel2, row.temp_amb, row.temp_panel1,  
                row.temp_panel2, row.vel_viento, row.dir_viento, row.hum_rel))
            conn.commit()
        except AttributeError:
            # En caso de que alguna de las columnas no exista porque el source no las incluya, se inserta el registro sin ella
            cur.execute(f"""INSERT INTO {schema_name}.meteo_raw(
                    dispositivo_id, datetime_utc, med_id, status, alarma, rad_poa, rad_hor, rad_celda1,
                    rad_celda2, temp_amb, temp_panel1, temp_panel2, vel_viento, dir_viento, hum_rel)
                VALUES 
                    (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);""",
                (row.dispositivo_id, row.datetime, row.med_id, row.status, row.alarma, row.rad_poa, row.rad_hor, 
                row.rad_cel1, row.rad_cel2, row.temp_amb, row.temp_panel1, row.temp_panel2, row.vel_viento, 
                row.dir_viento, row.hum_rel))
            conn.commit()
