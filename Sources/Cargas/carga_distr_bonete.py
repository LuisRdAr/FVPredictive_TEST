...
if __name__ == "__main__":
    ...
    if params is None:
        ...
        sys.exit()
    data_path = os.path.join(root_path, "Documentacion/distribucion_modulos_inversores.csv")
        ...
        for row in df.itertuples():
            try:
                cur.execute(f"""INSERT INTO {schema_name}.distrib_inversores(
                        parque_id, subcampo, dispositivo_id, entrada_id, num_strings, potencia)
                    VALUES 
                        (%s, %s, %s, %s, %s, %s);""",
                    (row.parque_id, row.subcampo, row.dispositivo_id, row.entrada_id, row.num_strings, row.potencia))
                conn.commit()
