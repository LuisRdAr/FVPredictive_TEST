...
if __name__ == "__main__":
    ...
    df = procesar_directorio(data_path, 'cont')
    if df.empty:
    ...
    for row in tqdm(df.itertuples(), total = df.shape[0]):
        try:
            cur.execute(f"""INSERT INTO {schema_name}.contador_raw(
                    parque_id, descripcion_parque, localizacion_parque, potencia_max, 
                    num_paneles, dispositivo_id, nombre_dispositivo, ref, ubicacion,
                    descripcion_dispositivo, datetime_utc, med_id, status, energia_act_imp,
                    energia_act_exp, energia_rea_q1, energia_rea_q2, energia_rea_q3,
                    energia_rea_q4)
                VALUES 
                ...
                row.descripcion_dispositivo, row.datetime, row.med_id, row.status, row.energia_act_imp,
                row.energia_act_exp, row.energia_rea_q1, row.energia_rea_q2, row.energia_rea_q3,
                row.energia_rea_q4))
            conn.commit()
        except AttributeError:
            # En caso de que alguna de las columnas no exista porque el source no las incluya, se inserta el registro sin ella
            cur.execute(f"""INSERT INTO {schema_name}.contador_raw(
                    dispositivo_id, datetime_utc, med_id, status, energia_act_imp,
                    energia_act_exp, energia_rea_q1, energia_rea_q2, energia_rea_q3,
                    energia_rea_q4)
                VALUES 
                    (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);""",
                (row.dispositivo_id, row.datetime, row.med_id, row.status, row.energia_act_imp,
                row.energia_act_exp, row.energia_rea_q1, row.energia_rea_q2, row.energia_rea_q3,
                row.energia_rea_q4))
            conn.commit()
