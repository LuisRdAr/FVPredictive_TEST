...
# Adición de variables de intensidades de corrientes medias (planta e inversor) para un instante dado
amp_dc_inv = main_df[["dispositivo_id", "datetime_utc", "amp_dc"]].groupby(["dispositivo_id", "datetime_utc"]).mean().reset_index().rename(columns={"amp_dc": "amp_dc_inv"})
amp_dc_planta = main_df[["datetime_utc", "amp_dc"]].groupby(["datetime_utc"]).mean().reset_index().rename(columns={"amp_dc": "amp_dc_planta"})
...
for inv_id in np.sort(main_df["dispositivo_id"].unique()):
    ...
    if optimizacion:
        ...
        if torch.cuda.is_available() and model_name == "XGBRegressor" and flag:
            import cupy as cp
            device = 'cuda'
            train_set = (cp.asarray(element) for element in train_set)
            test_set = (cp.asarray(element) for element in test_set)
        else:
            device = 'cpu'
